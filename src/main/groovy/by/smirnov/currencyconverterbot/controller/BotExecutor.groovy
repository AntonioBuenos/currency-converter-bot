...
class BotExecutor {
    ...
    BotExecutor(@Lazy TelegramBot bot, MessageSender messageSender) {
        this.bot = bot
        this.messageSender = messageSender
    }

    void editMessage(String text, long chatId, int messageId) {
        try {
    ...
    void executeMessage(Message message, String messageText) {
        try {
    ...
    void executeMessage(EditMessageReplyMarkup message) {
        try {
            bot.execute(message)
        } catch (TelegramApiException e) {
